const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:4810';
...
const adminCookie = await login('admin@drone-robotics-inspection.local', 'admin123');
const managerCookie = await login('manager@drone-robotics-inspection.local', 'manager123');
const analystCookie = await login('analyst@drone-robotics-inspection.local', 'analyst123');
